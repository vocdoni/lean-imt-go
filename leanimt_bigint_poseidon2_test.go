...
// poseidon2 hash for *big.Int using your wrapper.
...
func TestInsertManyFiveLeavesAndUpdatesPoseidon2(t *testing.T) {
	const treeSize = 5
...
func TestProofsBigIntPoseidon2(t *testing.T) {
	// single leaf proof
...
func TestImportExportBigIntPoseidon2(t *testing.T) {
	leaves := []*big.Int{bigIntPoseidon(0), bigIntPoseidon(1), bigIntPoseidon(2), bigIntPoseidon(3), bigIntPoseidon(4)}
	tree1, _ := New(poseidonBig, BigIntEqualPoseidon, nil, nil, nil)
	if err := tree1.InsertMany(leaves); err != nil {
		t.Fatal(err)
